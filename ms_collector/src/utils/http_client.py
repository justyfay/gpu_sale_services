...
from src.schemas.additional_product_data_schema import AdditionalProductDataSchema
from src.schemas.main_product_data_schema import MainProductDataSchema
    ...
    def _make_request(self, url: str, headers: Dict = None) -> httpx.Response:
        for i in range(self.request_retries):
            try:
                ...
                if i == self.request_retries - 1:
                    raise httpx.ConnectError(
                        f"Failed try '{i + 1}' to connect '{url}' with timeout '{self.request_timeout}'. Error: {timeout_error}.'"
                    )
